...
## 1. Loading Libraries (also known as packages)
...
that all of the neccesary libraries are loaded prior to running code.

Also, not that there is a difference between loading a installing a library and 
loading a library. If this is your first time working in R, then you will probably need to install the firt library we will work with. Note that I have 
included the `install.packages("ggplot2")` code in this file, but we ususally
want to install packages in the console. 

We will learn how to plot data using the ggplot2 library. This library provides 
some more advanced features compared to the base plotting function that are 
alreay provided in R. 

```{r}
# install.packages("ggplot2") # installing libraries require double qoutations ""
...
You can insert a code chunk by clicking on the `insert` button in the top right
...
`control + enter (windows)` <br>
<br>
<br>

## 3. The very basics<br>
### Simple Caluations
```{r}
2 + 2
```
...
plot(pressure)<br>
<br>
In R, we put our arguments (which are extra things we want the function to do
or consider) inside these parentheses.
...
```{r}
# ts plus shift and tab gives you a timestamp comment
# Mon Mar  4 11:54:48 2019 ------------------------------
