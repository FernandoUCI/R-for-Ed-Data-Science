...
TO-DO Prior to Lesson
1. Create lesson 6 R-Markdown
...
```{r}
fulldata <- read_csv("~/OneDrive - personalmicrosoftsoftware.uci.edu/R-workshop/Data/Physics Course GB SV DEMO data.csv")
```
...
```{r}
barplot_color <- ggplot(fulldata_ss, aes(x=grade_lettergrade, fill=fulltimestatus)) + geom_bar()
barplot_color
```
...
```{r}
barplot_title <- barplot_color + labs(x="Grade", y = "Count", fill = "Full time status", caption = "Source: UCI Physic course data") + ggtitle ("Letter Grade Distribution by Full/Part Time Status ")
barplot_title
